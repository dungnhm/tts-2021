package com.tts.com.tts.maven.eclipse;
...
//		router.route().handler(new SessionsHandler());
		...
		httpServer.listen(result -> {
			if (result.failed()) {
				System.out.println("[INIT] START ORDER API ERROR " + result.cause());
			} else {
				System.out.println("[INIT] ORDER STARTED AT " + serverHost + " : "+ serverPort);
			}
		});
	}
